...
else
{
    ...
    while ($#working_queue != -1)
    {
        ...
        elsif ($m <= $n)
        {
            if ($sign eq "+")
            {
                #push(@pos_list, $current_call);
                my @text = split(/,/, $current_call);
                push(@pos_list, pop(@text));
            }
            else
            {
                #push(@neg_list, $current_call);
                my @text = split(/,/, $current_call);
                push(@neg_list, pop(@text));
            }
        }
    }
    #Reference info from:
...
open (FILE, '>>function_tree.txt');
    #print FILE join(";", @final_list).";";
    print FILE join(" ", sort(@pos_list))." ".join(" ", sort(@neg_list))."\n\n";
close (FILE);
